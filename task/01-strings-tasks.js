...
function encodeToRot13(str) {
    ...
    return arr.map((l) => {
        ...
        if (codeL >= minS && codeL <= maxS) {
            return String.fromCharCode((codeL - minS + 13) % (maxS - minS + 1) + minS);
        } else if (codeL >= minB && codeL <= maxB) {
            return String.fromCharCode((codeL - minB + 13) % (maxB - minB + 1) + minB);
        } else {
            return l;
        }
    }).join('')
